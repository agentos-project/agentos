...
agentos run papag_agent --registry-file /tmp/papag-components.yaml --function-name learn --arg-set-file $PAPAG_DIR/ppo_pong_args.yaml > papag_output.txt
AGENT_RUN_ID=$(grep -Eo 'AgentOutput ([0-9,a-f]+)' papag_output.txt|grep -o ' [0-9,a-f]\+')
...
agentos run sb3_agent --registry-file /tmp/sb3-agent-components.yaml --function-name learn --arg-set-file $SB3_DIR/ppo_pong_args.yaml > sb3_output.txt
AGENT_RUN_ID=$(grep -Eo 'AgentOutput ([0-9,a-f]+)' sb3_output.txt|grep -o ' [0-9,a-f]\+')
USE_LOCAL_SERVER=True agentos publish-run $AGENT_RUN_ID

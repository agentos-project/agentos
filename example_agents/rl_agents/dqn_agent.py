...
class EpsilonGreedyTFPolicy:
    def __init__(self, env_class):
        ...
        self.model = keras.Sequential([keras.layers.Dense(16,
    ...
    def compute_action(self, obs):
        self.age += 1
        epsilon = min(0.95, 1000.0 / self.age)  # decay epsilon so that we can converge to Q*
        if self.age % 5000 == 0:
        ...
        return np.argmax(self.model(obs[np.newaxis]))  # with 1-epsilon probability, use the policy
...
class DQNAgent(LearningAgent):
    def __init__(self, env_class, policy):
        ...
        self.memory_buffer = pd.DataFrame(columns=["states", "actions", "rewards", "next_states", "dones"])
        self.best_score, best_model = 0, None
        self.all_scores = []

    def train(self):
        for episode_num in range(self.num_episodes):
            traj = agentos.rollout(self.policy,
                                   self.env.__class__,
                                   max_steps=self.max_steps_per_episode)

            pre_state = traj.init_obs
            for i in range(len(traj.observations)):
                self.memory_buffer.loc[len(self.memory_buffer)] = [pre_state,
                                                                   traj.actions[i],
                                                                   traj.rewards[i],
                                                                   traj.observations[i],
                                                                   traj.dones[i]]
            ...
            if episode_num > 50:  # let memory build up a bit before starting to iterate on the model
                ...
                next_per_action_q_vals = self.policy.model(np.vstack(observations.next_states))
                next_q_vals = np.max(next_per_action_q_vals, axis=1)
                target_q_vals = observations.rewards + (1 - observations.dones) * self.discount_rate * next_q_vals


                def loss():
                    per_action_q_vals = self.policy.model(np.vstack(observations.states))
                ...
                self.policy.optimizer.minimize(loss, self.policy.model.trainable_variables)


if __name__ == "__main__":
    agentos.run_agent(DQNAgent, MountainCarEnv, EpsilonGreedyTFPolicy(MountainCarEnv))

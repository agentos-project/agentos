...
def init(name):
    ...
    """
    for file, content in INIT_FILES.items():
        f = open(file, "w")
...
@click.option("--hz", "-h", metavar="HZ", default=40, type=int,
              help="Frequency to call agent.step().")
@click.option("--max_steps", "-m", metavar="MAX_STEPS", type=int, default=None,
              help="Stop running agent after this many steps.")
def run(run_args, hz, max_steps):
    ...
    If no args are specified, look for default files defining agent:
        ...
        - else, look for file named `agent.py` in current working
          directory and then behave in the same was as if 1 argument
          (i.e., `agent.py`) was provided, as described below.
    Else, if 1 arg is specified, interpret it as `agent_filename`:
        - if it is a directory name, assume it is an AgentOS agent dir,
          ...
          and running `agentos run` (without arguments) in it.
        - if it is a file name, search that file for the first subclass of
    ...
    def _handle_single_run_arg(filename):
        ...
        agentos.run_agent(agent_cls, env_cls, hz=hz, max_steps=max_steps)
